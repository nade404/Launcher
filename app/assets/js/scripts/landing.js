...
function updateSelectedAccount(authUser) {
    ...
    if (authUser != null) {
        ...
        if (authUser.uuid != null) {
            document.getElementById('avatarContainer').style.backgroundImage = `url('https://crafatar.com/renders/body/${authUser.uuidMojang}')`
        }
    }
        ...
        if (m.context === 'validateJava') {
            if (m.result == null) {
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre EverShell.net, vous avez besoin de la version 64 bits de Java 8. Souhaitez-vous que nous en installions une copie? En installant, vous acceptez <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Oracle\' Le contrat de licence/a>.',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation x64 valide de Java 8 est requise pour démarrer.<br><br> Veuillez consulter notre <a href="https://github.com/ChickenDevLab/NexusLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java"> Guide de gestion Java </a> pour savoir comment installer manuellement Java.',
                            'I Understand',
                            'Go Back'
                        )
                        setOverlayHandler(() => {
            ...
            if (m.result === true) {
                ...
                setLaunchDetails('Java Herunterladen..')
                ...
                setOverlayContent(
                    ...
                    'Malheureusement nous avons rencontré un problème lors de la tentative d \' installation de Java. Vous devrez installer manuellement une copie. Veuillez consulter notre <a href="https://github.com/ChickenDevLab/NexusLauncher/wiki">Troubleshooting Guide</a> for more details and instructions.',
                    'J\'ai Compris'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
...
function dlAsync(login = true) {
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur lors du démarrage du jeu', err.message || 'Veuillez le signaler sur notre discord.')
    })
    aEx.on('close', (code, signal) => {
        if (code !== 0) {
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Erreur lors du démarrage du jeu, veuillez le signaler sur notre discord')
        }
    })
        ...
        if (m.context === 'validate') {
            switch (m.data) {
                case 'distribution':
                    ...
                    setLaunchDetails('Charger les informations de version.')
                    break
                case 'version':
            ...
            switch (m.data) {
                case 'download':
                    ...
                    setLaunchDetails('Préparez-vous à décoller.')
                    break
            }
        } else if (m.context === 'error') {
            cnt = cnt - 1
            switch (m.data) {
                case 'download':
                    ...
                    if (m.error.code === 'ENOENT') {
                        showLaunchFailure(
                            '',
                            'Impossible de télécharger les fichiers depuis les serveurs. Assurez-vous que vous êtes connecté à Internet et réessayez.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur de téléchargement',
                            'Erreur lors du démarrage du jeu, veuillez le signaler sur notre discord'
                        )
                    }
            ...
            if (login && allGood) {
                ...
                loggerLaunchSuite.log(`Envoi du compte sélectionné (${authUser.displayName}) pour ProcessBuilder.`)
                let pb = new ProcessBuilder(serv, versionData, authUser, remote.app.getVersion())
                setLaunchDetails('Démarrer jeu ..')
                ...
                const gameStateChange = function (data) {
                    DiscordWrapper.updateDetails('Jouez à Minecraft sur EverShell.net')
                }

                const gameErrorListener = function (data) {
                    data = data.trim()
                    if (data.indexOf('Impossible de trouver ou de charger la classe principale') > -1) {
                        loggerLaunchSuite.error('Le lancement du jeu a échoué, la classe principale n\'a pas été trouvée.')
                        showLaunchFailure('Erreur lors du démarrage du jeu', 'La classe principale n\'a pas pu être chargée. Veuillez en informer Beldah')
                    }
                }
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if (DistroManager.getDistribution() == null) {
                showLaunchFailure('Fataler Fehler', 'Erreur lors du démarrage du jeu, veuillez le signaler sur notre discord')
