        ...
        val.onclick = (e) => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le launcher, vous avez besoin au moins de un compte. Vous devrez vous reconnecter plus tard. <br> <br> Voulez-vous vraiment vous déconnecter?',
                    'Oui je suis sûr!',
                    'Non, annulez!'
                )
                setOverlayHandler(() => {
...
function refreshAuthAccountSelected(uuid) {
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if (uuid === val.getAttribute('uuid')) {
            ...
            selBtn.innerHTML = 'Choisissez votre compte'
        }
    })
}
    ...
    authKeys.map((val) => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.displayName}" src="https://crafatar.com/renders/body/${acc.uuidMojang}?scale=3&default=MHF_Steve&overlay">
            </div>
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte Actuel &#10004;' : '>Choisir un compte'}</button>
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if (v.valid) {
            if (v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Selection invalide:  Java ${v.version.major} Update ${v.version.update} (x${v.arch})`
            } else {
                settingsJavaExecDetails.innerHTML = `Selection invalide: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})`
            }
        } else {
    ...
    if (data != null) {
        ...
        settingsUpdateButtonStatus('Rechercher des mises à jour', false, () => {
            if (!isDev) {
                ...
                settingsUpdateButtonStatus('Vérifiez les mises à jour.', true)
            }
        })
    }
}
